"""Regression: /api/face/verify must ignore client-supplied match thresholds."""
...
def _rag_import_stubs() -> dict[str, ModuleType]:
    """Avoid pulling LlamaIndex when importing the bridge entrypoint."""
    ...
    def test_verify_ignores_client_supplied_threshold(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch.dict(os.environ, env):
                ...
                for mod in ("stitch_rag_bridge", "bridge.face_routes", "bridge.rag_routes"):
                    sys.modules.pop(mod, None)
                with patch.dict(sys.modules, _rag_import_stubs()):
                    import bridge.face_routes as face_routes
                    import stitch_rag_bridge  # noqa: F401 — registers blueprints on app
                ...
                # Client tries both a near-zero float and a string "0" (the latter
                # bypassed the old `x or DEFAULT` expression).
                with patch.object(face_routes, "_face_imports", side_effect=fake_face_imports):
                    client = stitch_rag_bridge.app.test_client()
                    for client_threshold in (0.01, "0"):
                        thresholds.clear()
                        resp = client.post(
                            "/api/face/verify",
                            json={
                        ...
                        self.assertEqual(resp.status_code, 200, msg=payload)
